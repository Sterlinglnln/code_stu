#include <iostream> // This line includes the iostream library
using namespace std; // This line allows us to use standard names from the std name

int main() {
    cout << "Hello, world again!" << endl; // This line prints "Hello, World!" to the console
    system("pause"); // This line is used to pause the console window in some environments
    cin.get(); // Wait for user input before closing the console window
    return 0; // Return 0 indicates successful completion of the program
}
